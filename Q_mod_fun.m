...
% and all years (mode = 3), depending on the mode given.
    ...
    if mode == 0   %model for every hour
        ...
        Q_mod = zeros(length(Q_obs),1);
        Q_b = Q_b.*ones(length(Q_obs),1);
        Qsub = zeros(length(Q_obs),1);
        Qsup = zeros(length(Q_obs),1);
        qsub = zeros(length(Q_obs),1);
        qsup = zeros(length(Q_obs),1);
        ...
        for y=1:Nyears
            for m=1:12
                for h=month_start_hour(m):month_end_hour(m)
                    ...
                    %disp(t)
                    % Infiltration
                    I(t) = min(P(t), K_sat); %[m/h]
                    % Runoff
                    R(t) = P(t)-I(t);       %[m/h]
                    
                    
                    if s(t)<=sw && s(t)>=0
                        ET_hour_year(t)= 0; % [m/h]
                        %disp('1st case')
                    elseif s(t)<=s1 && s(t)>sw
                        ET_hour_year(t)= kc(m)*ET_0(m)*((s(t)-sw)/(s1-sw)); % [m/h]
                        %disp('2nd case')
                    elseif s(t)<=1 && s(t)>s1
                        ET_hour_year(t)= kc(m)*ET_0(m); % [m/h]
                        %disp('3rd case')
                    elseif s(t) > 1
                        disp('Warning : s > 1')
                    elseif s(t)<0
                    ...
                    % Leaching
                    L(t)=K_sat*(s(t)^c); % [m/h]
                    % s soil moisture, euler forward method
